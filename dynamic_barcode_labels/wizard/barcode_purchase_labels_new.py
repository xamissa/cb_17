...
class BarcodePurchaseLabelsWiz(models.TransientModel):
    _name = "barcode.purchase.labels.wiz.new"
    _description = 'Barcode Product Labels Wizard'

    product_barcode_ids = fields.One2many('barcode.purchase.labels.wiz.line.new', 'label_id', 'Product Barcode')
    ...
    def print_barcode_labels(self):
        ...
        barcode_lines = self.env['barcode.purchase.labels.wiz.line.new'].browse(data['barcode_labels'])
        datas = {
             'ids': [1],
             'model': 'barcode.sale.labels.wiz.new',
             'form': data
        }
        return self.env.ref('dynamic_barcode_labels.printed_purchase_order_barcode_labels_id_new').report_action(barcode_lines, data=datas)


class BarcodePurchaseLabelsLine(models.TransientModel):
    _name = "barcode.purchase.labels.wiz.line.new"
    _description = 'Barcode Product Labels Line'
    
    label_id = fields.Many2one('barcode.purchase.labels.wiz.new', 'Barcode labels')
