...
class BarcodeStockLabelsWiz(models.TransientModel):
    _name = "barcode.stock.labels.wiz.new"
    _description = 'Barcode Product Labels Wizard'

    product_barcode_ids = fields.One2many('barcode.stock.labels.wiz.line.new', 'label_id', 'Product Barcode')
    ...
    def print_barcode_labels(self):
        ...
        barcode_lines = self.env['barcode.stock.labels.wiz.line.new'].browse(data['barcode_labels'])
        datas = {
             'ids': [1],
             'model': 'barcode.stock.labels.wiz.new',
             'form': data
        }
        return self.env.ref('dynamic_barcode_labels.printed_stock_picking_barcode_labels_id_new').report_action(barcode_lines, data=datas)


class BarcodeStockLabelsLine(models.TransientModel):
    _name = "barcode.stock.labels.wiz.line.new"
    _description = 'Barcode Product Labels Line'
    
    label_id = fields.Many2one('barcode.stock.labels.wiz.new', 'Barcode labels')
