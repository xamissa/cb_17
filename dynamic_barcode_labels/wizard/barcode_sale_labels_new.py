...
class BarcodeSaleLabelsWiz(models.TransientModel):
    _name = "barcode.sale.labels.wiz.new"
    _description = 'Barcode Product Labels Wizard'

    product_barcode_ids = fields.One2many('barcode.sale.labels.wiz.line.new', 'label_id', 'Product Barcode')
    ...
    def print_barcode_labels(self):
        ...
        barcode_lines = self.env['barcode.sale.labels.wiz.line.new'].browse(data['barcode_labels'])
        datas = {
             'ids': [1],
             'model': 'barcode.sale.labels.wiz.new',
             'form': data
        }
        return self.env.ref('dynamic_barcode_labels.printed_sale_order_barcode_labels_id_new').report_action(barcode_lines, data=datas)


class BarcodeSaleLabelsLine(models.TransientModel):
    _name = "barcode.sale.labels.wiz.line.new"
    _description = 'Barcode Product Labels Line'
    
    label_id = fields.Many2one('barcode.sale.labels.wiz.new', 'Barcode labels')
