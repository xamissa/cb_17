...
class BarcodeProductLabelsTempWiz(models.TransientModel):
    _name = "barcode.product.template.labels.wiz.extra"
    _description = 'Extra Barcode Product Template Labels Wizard'

    product_barcode_ids = fields.One2many('barcode.product.template.labels.wiz.line.extra', 'label_id', 'Product Barcode')
    ...
    def print_barcode_labels(self):
        ...
        barcode_lines = self.env['barcode.product.template.labels.wiz.line.extra'].browse(data['barcode_labels'])
        datas = {
             'ids': [1],
             'model': 'barcode.product.template.labels.wiz.extra',
             'form': data
        }
        return self.env.ref('dynamic_barcode_labels.printed_product_temp_barcode_labels_extra_id').report_action(barcode_lines, data=datas)


class BarcodeProductLabelsLine(models.TransientModel):
    _name = "barcode.product.template.labels.wiz.line.extra"
    _description = 'Extra Barcode Product Labels Line'
    
    label_id = fields.Many2one('barcode.product.template.labels.wiz.extra', 'Barcode labels')
